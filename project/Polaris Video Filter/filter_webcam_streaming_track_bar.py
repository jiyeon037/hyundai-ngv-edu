# 1개의 트랙바로 2개의 기준을 공통적으로 적용하는 코드
...
cv2.createTrackbar('threshold','Binary', 0, 255, nothing) #트랙바를 이름이'Binary'인 윈도우에 붙임'
cv2.setTrackbarPos('threshold', 'Binary', 127) #초기값 127
...
def Filter(img, value = 0): # 픽셀 값 평균을 기준으로 필터링
    ...
    if value == 0:
        ret,img_filter = cv2.threshold(img, low, 255, cv2.THRESH_BINARY)
    else:
...
video_capture_2 = cv2.VideoCapture(1) # 외장형 USB 웹캠2
...
while True:
    ...
    if (ret1 and ret2):
        ...
        A = pixel_ratio(img1, img2) # 대입인자는 이미 0 또는 1만 있는 이미지만
        print(A)       
